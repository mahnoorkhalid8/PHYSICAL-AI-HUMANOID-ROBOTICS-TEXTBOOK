#!/usr/bin/env python3
"""
Generate a fresh JWT token with current timestamp and 30-minute expiration
"""
import jwt
import json
...
# Define the payload for a fresh token
...
payload = {
    ...
    "iat": int(current_time.timestamp()),  # Issued at (current time)
    "exp": int((current_time + timedelta(minutes=30)).timestamp()),  # Expires in 30 minutes
    "nbf": int(current_time.timestamp()),  # Not before (current time)
...
print("Generated Fresh JWT Token (RS256) - Valid for 30 minutes:")
print(token)
print("\nPayload:")
